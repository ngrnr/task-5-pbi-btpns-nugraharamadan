...
import (
	...
	photoRes "github.com/ngrnr/task-5-pbi-btpns-nugraharamadan/app/photo"
...
func (h *photoController) Get(c *gin.Context) {
	...
	if err != nil {
		response := helpers.ApiResponse(http.StatusBadRequest, "error", nil, "Failed to Get Photo")
		c.JSON(http.StatusBadRequest, response)
		return
	}

	if userPhoto.PhotoURL == "" {
		response := helpers.ApiResponse(http.StatusBadRequest, "error", nil, "Please Upload Photo")
		c.JSON(http.StatusBadRequest, response)
		return
	}

	formatter := photoRes.FormatPhoto(&userPhoto, "")
	response := helpers.ApiResponse(http.StatusOK, "success", formatter, "Photo Successfully Fetched")
...
func (h *photoController) Create(c *gin.Context) {
	...
	if countPhoto > 0 {
		...
		response := helpers.ApiResponse(http.StatusBadRequest, "error", data, "Photo Already Uploaded")
		c.JSON(http.StatusBadRequest, response)
		return
	}
	...
	if err != nil {
		...
		response := helpers.ApiResponse(http.StatusUnprocessableEntity, "error", errorMessages, "Failed to Upload Photo")
		c.JSON(http.StatusBadRequest, response)
		return
	}
	...
	if err != nil {
		...
		response := helpers.ApiResponse(http.StatusUnprocessableEntity, "error", data, "Failed to Upload Photo")
		c.JSON(http.StatusUnprocessableEntity, response)
		return
	}
	...
	response := helpers.ApiResponse(http.StatusOK, "success", data, "Photo Uploaded Successfully")
	c.JSON(http.StatusOK, response)
}
...
func (h *photoController) Update(c *gin.Context) {
	...
	if err != nil {
		response := helpers.ApiResponse(http.StatusBadRequest, "error", err, "Failed to Update Photo")
		c.JSON(http.StatusBadRequest, response)
		return
	}
	...
	if err != nil {
		...
		response := helpers.ApiResponse(http.StatusUnprocessableEntity, "error", data, "Failed to Update Photo")
		c.JSON(http.StatusUnprocessableEntity, response)
		return
	}
	...
	err = c.SaveUploadedFile(file, path)
	if err != nil {
		response := helpers.ApiResponse(http.StatusBadRequest, "error", err, "Failed to Upload Photo")
		c.JSON(http.StatusBadRequest, response)
		return
	}
	...
	response := helpers.ApiResponse(http.StatusOK, "success", data, "Profile Updated Successfully")
	c.JSON(http.StatusOK, response)
}
...
func (h *photoController) Delete(c *gin.Context) {
	...
	if err != nil {
		...
		response := helpers.ApiResponse(http.StatusBadRequest, "error", data, "Failed deleting photo")
		c.JSON(http.StatusBadRequest, response)
		return
	}
	...
	response := helpers.ApiResponse(http.StatusOK, "success", data, "Photo Deleted Successfully")
	c.JSON(http.StatusOK, response)
}
